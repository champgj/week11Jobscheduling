...
![123](https://user-images.githubusercontent.com/75067408/118921270-de2f8a80-b972-11eb-8edb-0973b1187bfd.png)
...
**초과는 java.lang.OutOfMemoryError: Java heap space (메모리초과!)를 말합니다.**
...
![image-20210519022134695](https://user-images.githubusercontent.com/75067408/118921297-e982b600-b972-11eb-8ada-371877715da0.png)
...
그래서 Greedy와는 비교할 필요없을 정도로 극심한 차이가 많이 난다.

## p.s.) 근사알고리즘을 짜면서 추가적으로 알게된 것들. .

근사 알고리즘 중 위에 구현한 Job scheduling 은 실제로 load balancer 알고리즘으로도 사용된다고 한다. 

요청이 들어온 시점에서 가장 적은 연결 상태를 보이는 서버에 트래픽을 배정하는 방식이고, 

서버에 분배된 트래픽들이 일정하지 않은 경우에 더 적합하다고 한다.

실제로 증가된 트래픽을 해결해야하는 경우에 scale up과 scale out을 통해 해결할 수 있는데

쉽게 말해 scale up은 서버 자체의 성능을 높이는 것이고 scale out은 여러대의 서버를 두는 것이다.

그래서 scale out을 할 때 여러대의 서버로 트래픽을 균등하게 분산해주는 로드밸런싱이 필요하다

이 로드 밸런싱 알고리즘 중에서도 라운드로빈, 가중 라운드로빈, 최소연결방식, ip 해시 방식 등이 있다.



- 라운드로빈 : 서버에 들어온 요청을 순서대로 돌아가며 배정
- 가중 라운드로빈 방식 : 각 서버에 가중치를 매긴 후 가중치가 높은 서버에 요청을 우선적으로 배정

- 최소 연결 방식 : 요청이 들어온 시점에 가장 적은 연결 상태를 보이는 서버에 트래픽을 배정

- IP 해시 방식 : 클라이언트의 IP주소를 특정 서버로 매핑하여 처리



이 있다. 그 중 위에서 구현한 코드는 가중치가 없고 작업의 시간만 주어지기 때문에
...
**추가로 손으로 풀어본 greedy와 bruteforce의 작업처리 비교이다.**

![image](https://user-images.githubusercontent.com/75067408/118922854-ad048980-b975-11eb-95cb-9895b3dfe53e.png)
